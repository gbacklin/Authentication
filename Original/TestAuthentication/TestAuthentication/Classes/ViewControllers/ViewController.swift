...
class ViewController: UIViewController {
    ...
    var state = AuthenticationState.loggedout {
        ...
        didSet {
            ...
            faceIDLabel.isHidden = (state == AuthenticationState.loggedin) || (!Authentication.shared.isFaceId())
        }
    }
    ...
    @IBAction func tapButton(_ sender: UIButton) {
        Authentication.shared.login {[unowned self] (result, error, state) in
            self.state = state
            if result == false {
                let alertController = Authentication.shared.createAlertController(error: error, title: "Login")
